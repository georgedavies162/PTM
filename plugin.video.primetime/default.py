...
#############Defined Strings#############
addon_id     = 'plugin.video.primetime'
...
series_api   = '%s:%s/enigma2.php?username=%s&password=%s&type=get_series'%(host,port,username,password)
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.primetime/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.primetime/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		auth = tools.OPEN_URL(auth)
		if not auth=="":
			tools.addDir('----------------------------','url',6,icon,fanart,'')
			tools.addDir('Live TV Categorys','live',1,icon,fanart,'')
			tools.addDir('Catchup TV','url',12,icon,fanart,'')
			#tools.addDir('Sport Listings','SPORT_LISTINGS',98,icon,fanart,'')
			...
			tools.addDir('On Demand Content','vod',3,icon,fanart,'')
			tools.addDir('TV Shows','series',99,icon,fanart,'')
			tools.addDir('Run a Speed Test','ST',10,icon,fanart,'')
			tools.addDir('Extras','url',16,icon,fanart,'')
			tools.addDir('----------------------------','url',8,icon,fanart,'')
...
def livecategory(url):
	...
	for a in all_cats:
		...
		tools.addDir(name.replace('All','--[COLORpuple] F[/COLOR]ull Channel List --'),url1,2,icon,fanart,'')
...
def Livelist(url):
	...
	for a in all_cats:
		...
		tools.addDir(name.replace('-','[COLOR yellow] -[/COLOR]'),url1,4,thumb,fanart,base64.b64decode(desc))
...
def series(url):
	if url =="series":
		open = tools.OPEN_URL(series_api)
	else:
		open = tools.OPEN_URL(url)
	all_cats = tools.regex_get_all(open,'<channel>','</channel>')
	for a in all_cats:
...
##########################################
def catchup():
    loginurl   = "http://prismplus.cf:5678/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("[COLOR blue]primetime[/COLOR]", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search [COLOR blue]primetime[/COLOR]:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('[COLOR blue]primetime[/COLOR]', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('[COLOR blue]primetime[/COLOR]', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('[COLOR blue]primetime[/COLOR]', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('[COLOR blue]primetime[/COLOR]', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('[COLOR blue]primetime[/COLOR]', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.primetime/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('[COLOR blue]primetime[/COLOR]', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def accountinfo():
	...
	try:
		...
		for day,month,year in expreg:
			...
			tools.addDir('[COLOR blue]U[/COLOR]sername : '+username,'','',icon,fanart,'')
			tools.addDir('[COLOR blue]P[/COLOR]assword : '+password,'','',icon,fanart,'')
			tools.addDir('[COLOR blue]E[/COLOR]xpiry Date: '+expiry,'','',icon,fanart,'')
			tools.addDir('[COLOR blue]A[/COLOR]ccount Status : %s'%status,'','',icon,fanart,'')
			tools.addDir('[COLOR blue]C[/COLOR]urrent Connections: '+ active,'','',icon,fanart,'')
			tools.addDir('[COLOR blue]A[/COLOR]llowed Connections: '+connects,'','',icon,fanart,'')
			tools.addDir('[COLOR blue]L[/COLOR]ocal IP Address: '+ip,'','',icon,fanart,'')
			tools.addDir('[COLOR blue]E[/COLOR]xternal IP Address: '+extip,'','',icon,fanart,'')
	except:pass
		
	
def correctPVR():

	addon = xbmcaddon.Addon('plugin.video.primetime')
	...
	loginurl   = "http://prismplus.cf:5678/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://prismplus.cf:5678/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('[COLOR blue]primetime[/COLOR]','Would You like us to Setup the TV Guide for You?')
		if dialog:
			dialog = xbmcgui.Dialog().select('Select a TV Guide to Setup', ['iVue TV Guide','PVR TV Guide','Both'])
			if dialog==0:
				ivueint()
